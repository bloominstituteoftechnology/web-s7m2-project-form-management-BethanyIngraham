...
//this will get you a fresh obj. every time you intialize state

export default function App() {
  const [members, setMembers] = useState(teamMembers)
  const [editing, setEditing] = useState(null)
  //null means the form is in creation mode
  //a number means we are editing the member by id
  ...
  useEffect(() => {
    ...
    if(editing === null){
      ...
      const {fname, lname, bio} = members.find(mem => mem.id === editing) //need help here
  ...
  const onChange = evt => {
   ...
   // debugger - to check what happened
   let {id, value} = evt.target;
   setValues(previousValues => ({...previousValues, [id]: value})) 
   //spread current values, don't override inputs, when id changes put it's value into state
   //this passes a callback that takes previous values and returns a new state
   //this is computing new state off of current state
   //state setters can take a fresh new state or a callback which returns new state
  }
  const edit = id => {
  ...
  const submitNewMember = () => {
    ...
    const newMember = {fname, lname, bio, id:getId()} //creating a new member
    //when se set state, we need to create a whole new state!
    setMembers([...members, newMember]) //use an array for a list of data
  }
  const editExistingMember = () => {
    ...
    setMembers(prevMembers => prevMembers.map(mem => {
      ...
      return mem //need help understanding this!
    }))
    setEditing(null)
  }
  const onSubmit = evt => {
